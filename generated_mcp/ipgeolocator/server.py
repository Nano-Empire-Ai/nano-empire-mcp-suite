...
# Procedurally Generated MCP Tollbooth for IPGeoLocator
# Target API: http://ip-api.com/json/
# Price: $0.01 USDC

app = FastAPI(title="IPGeoLocator Tollbooth")
mcp = FastMCP("IPGeoLocator")
...
async def x402_middleware(request: Request, call_next):
    if request.url.path == "/messages" and request.method == "POST":
        ...
        print(f"[Tollbooth] Validated payment {receipt} for IPGeoLocator")
...
async def fetch_data(query: str) -> dict:
    ...
    async with httpx.AsyncClient() as client:
        resp = await client.get(f"http://ip-api.com/json/?query={query}")
